...
class SceneManager {
    ...
    setupXR() {
        try {
            ...
            for (let i = 0; i < 2; i++) {
                ...
                // Add a SUPER VISIBLE laser beam - MUCH LONGER for visibility in space
                const laserLength = 500; // 500m long laser for visibility
                const laserGeometry = new THREE.CylinderGeometry(0.02, 0.02, laserLength, 8);
                const laserMaterial = new THREE.MeshBasicMaterial({ 
                    ...
                    toneMapped: false // Always visible
                });
                
                const laser = new THREE.Mesh(laserGeometry, laserMaterial);
                laser.rotation.x = Math.PI / 2; // Rotate to point forward
                laser.position.set(0, 0, -laserLength/2); // Position at center
                laser.name = 'laser';
                ...
                pointer.position.set(0, 0, -laserLength);
                pointer.name = 'pointer';
                controller.add(pointer);
                
                // Add outer glow to pointer - BIGGER
                ...
                // Add a cone at controller tip for direction indicator - BIGGER
                const coneGeometry = new THREE.ConeGeometry(0.05, 0.8, 8);
            ...
            this.renderer.xr.addEventListener('sessionstart', () => {
                ...
                // SHOW VR UI panel by default with helpful message
                if (this.vrUIPanel) {
                    this.vrUIPanel.visible = true;
                    this.updateVRStatus('🎮 VR Mode Active! Use GRIP button to toggle menu');
                }
            });
    ...
    handleVRAction(action) {
        ...
        switch(action) {
            ...
            case 'speedmode':
                if (app.topicManager && app.topicManager.solarSystemModule) {
                    const module = app.topicManager.solarSystemModule;
                    const newMode = module.toggleSpeedMode();
                    this.updateVRStatus(`🚀 ${newMode === 'realistic' ? 'Real-time' : 'Educational'} Speed`);
                }
                break;
            case 'reset':
                this.resetCamera();
    ...
    updateXRMovement() {
        ...
        for (let i = 0; i < inputSources.length; i++) {
            ...
            // Always moves toward WHERE YOU'RE LOOKING (head-relative)
            // ============================================
            if (handedness === 'left') {
                const baseSpeed = 0.5 * sprintMultiplier; // Increased for space scale
                
                // Forward/Backward & Strafe
                if (Math.abs(stickX) > deadzone || Math.abs(stickY) > deadzone) {
                    // FORWARD/BACKWARD: Push stick FORWARD to move toward where you're looking
                    // Use camera direction (where HMD is facing), not dolly rotation
                    const forwardMovement = cameraDirection.clone();
                    forwardMovement.y = 0; // Stay horizontal (no flying up/down)
                    forwardMovement.normalize();
                    
                    // Negative stickY = push forward = move forward (intuitive!)
                    this.dolly.position.add(forwardMovement.multiplyScalar(-stickY * baseSpeed));
                    
                    // STRAFE LEFT/RIGHT relative to where you're looking
...
const SOLAR_SYSTEM_CONFIG = {
    PERFORMANCE: {
        updateIntervals: {
            solarFlares: 2,          // Update sun flares every N frames
            starTwinkling: 5,        // Update stars every N frames
            cometDustTail: 3,        // Update comet dust tail every N frames
            cometIonTail: 2,         // Update comet ion tail every N frames
            moonDebugLog: 1000       // Log moon position 1 in N updates
        },
        ...
        lodDistances: {              // Level of Detail distances
            high: 100,               // Within 100 units = high detail
    ...
    SPEED_MODES: {
        // REALISTIC: Real-time orbital periods (will appear very slow)
        realistic: {
            mercury: 0.00416,   // 88 Earth days
            venus: 0.00162,     // 225 Earth days
            earth: 0.001,       // 365.25 days
            moon: 0.0367,       // 27.3 days around Earth
            mars: 0.000531,     // 687 days
            jupiter: 0.0000843, // 4,333 days (11.86 years)
            saturn: 0.0000339,  // 10,759 days (29.46 years)
            uranus: 0.0000119,  // 30,687 days (84.02 years)
            neptune: 0.00000606 // 60,190 days (164.79 years)
        },
        // EDUCATIONAL: Visible orbits for learning (default)
        educational: {
            mercury: 0.02,      // Visible in minutes
            venus: 0.015,       // Medium speed
            earth: 0.01,        // Good reference
            moon: 0.05,         // Visible orbit around Earth
            mars: 0.008,        // Slower than Earth
            jupiter: 0.005,     // Slow giant
            saturn: 0.004,      // Very slow
            uranus: 0.003,      // Extremely slow
            neptune: 0.002      // Slowest planet
        }
    }
};
...
class SolarSystemModule {
    constructor(uiManager) {
        ...
        // Speed mode: 'educational' = visible orbits, 'realistic' = real-time
        this.speedMode = 'educational'; // Default to educational for visibility
    ...
    update(deltaTime, timeSpeed, camera, controls) {
        ...
        Object.values(this.planets).forEach(planet => {
            if (planet && planet.userData) {
                ...
                if (planet.userData.clouds) {
                    planet.userData.clouds.rotation.y += planet.userData.rotationSpeed * rotationSpeed * SOLAR_SYSTEM_CONFIG.ANIMATION.cloudRotationMultiplier;
                }
        ...
        if (this.kuiperBelt) {
            this.kuiperBelt.rotation.y += SOLAR_SYSTEM_CONFIG.ANIMATION.kuiperBeltRotation * timeSpeed;
        }
        ...
        if (this.sun) {
            ...
            // Animate solar flares (optimized - update every N frames)
            const flareInterval = SOLAR_SYSTEM_CONFIG.PERFORMANCE.updateIntervals.solarFlares;
            if (this.sun.userData.flares && this._sunFlareFrame % flareInterval === 0) {
        ...
        // Twinkle stars slightly (optimized - only every N frames)
        const starInterval = SOLAR_SYSTEM_CONFIG.PERFORMANCE.updateIntervals.starTwinkling;
        if (this.starfield && this._starTwinkleFrame % starInterval === 0 && Math.random() < 0.3) {
            const sizes = this.starfield.geometry.attributes.size.array;
            const twinkleCount = SOLAR_SYSTEM_CONFIG.PERFORMANCE.starTwinkleCount;
            
            // Update only a subset of stars for performance
    ...
    toggleSpeedMode() {
        // Toggle between educational and realistic speed
        this.speedMode = this.speedMode === 'educational' ? 'realistic' : 'educational';
        console.log(`🚀 Speed mode: ${this.speedMode.toUpperCase()}`);
        
        // Update all planet speeds
        const speeds = SOLAR_SYSTEM_CONFIG.SPEED_MODES[this.speedMode];
        Object.entries(this.planets).forEach(([name, planet]) => {
            if (planet && planet.userData && speeds[name]) {
                planet.userData.baseSpeed = speeds[name];
            }
        });
        
        // Update moon speed (relative to its planet)
        Object.values(this.moons).forEach(moon => {
            if (moon && moon.userData) {
                // Moon speed is stored separately
                moon.userData.baseSpeed = speeds.moon || 0.05;
            }
        });
...
class TopicManager {
    ...
    setupControls() {
        ...
        // Speed mode toggle button (NEW!)
        const speedModeButton = document.getElementById('toggle-speed-mode');
        if (speedModeButton) {
            speedModeButton.addEventListener('click', () => {
                if (this.solarSystemModule) {
                    const newMode = this.solarSystemModule.toggleSpeedMode();
                    speedModeButton.classList.toggle('toggle-on', newMode === 'realistic');
                    speedModeButton.textContent = newMode === 'realistic' ? 
                        '🕐 Realistic Speed' : '🚀 Educational Speed';
                    
                    // Show notification
                    if (this.uiManager) {
                        this.uiManager.showNotification(
                            newMode === 'realistic' ? 
                            '🕐 Real-time orbital speeds (very slow!)' : 
                            '🚀 Educational speeds (visible orbits)'
                        );
                    }
                }
            }, { passive: true });
